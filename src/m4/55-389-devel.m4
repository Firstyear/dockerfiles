...
# `RUN' /usr/bin/DNFYUM install -y pytest python-ldap python-flask krb5-server-ldap krb5-server httpd httpd-devel
...
# `RUN' /usr/bin/DNFYUM debuginfo-install -y audit-libs cyrus-sasl-gssapi cyrus-sasl-lib cyrus-sasl-md5 keyutils-libs krb5-libs libcom_err libdb libevent libgcc libicu libselinux libstdc++ libtalloc libtevent nspr nss nss-util openldap openssl-libs pam pcre svrcore zlib; true && /usr/bin/DNFYUM clean all
